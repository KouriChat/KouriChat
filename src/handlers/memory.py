...
class MemoryHandler:
    ...
    def add_short_memory(self, message: str, reply: str, user_id: str, group_id: str = None, sender_name: str = None):
        ...
        try:
            ...
            # 获取记忆文件路径
            short_memory_path, _ = self._get_memory_paths(user_id, group_id, sender_name)
            ...
            # 构建记忆键，包含群聊和用户信息
            memory_key = f"[{timestamp}] 对方"
            if group_id and sender_name:
                memory_key += f"({sender_name}@{group_id})"
            memory_key += f": {message}"
    ...
    def _ensure_memory_files(self, user_id: str, group_id: str = None, sender_name: str = None):
        ...
        try:
            # 获取所有记忆文件路径
            short_memory_path, long_memory_buffer_path = self._get_memory_paths(user_id, group_id, sender_name)
            ...
            # 检查并创建所有必要的文件
            for file_path in [short_memory_path, long_memory_buffer_path]:
    ...
    def get_relevant_memories(self, query: str, user_id: Optional[str] = None, group_id: str = None, sender_name: str = None) -> List[str]:
        ...
        _, long_memory_buffer_path = self._get_memory_paths(user_id, group_id, sender_name)
    ...
    def summarize_daily_memory(self, user_id: str):
        """将短期记忆总结为日记式的长期记忆"""
        try:
            short_memory_path, long_memory_buffer_path = self._get_memory_paths(user_id)
    ...
    def _init_user_files(self, user_id: str):
        ...
        try:
            ...
            files_to_check = [short_memory_path, long_memory_buffer_path]
